...
try{
    $s = $pdo->prepare("select mainID from shadowIDs where shadowuser=:user");
...
if($s->rowCount()>0){
    while($row = $s->fetch()){
        $shadowIDs [] = $row['mainID'];
    }
    $mainID = $shadowIDs[0];
    $main_user = json_decode(@file_get_contents("https://hivaind.ir/wil/informationID.php?id=$mainID"))->usr;
}
...
if($main_user == ""){
    ...
    $api_url = "http://hivaind.ir/property/user-check.php?usr=" . strval($main_user);
    $json = @file_get_contents($api_url);
    if($json == "" or $json == FALSE){
        $json = @file_get_contents("https://hivaindbackup.ir/property/user-check.php?usr=" . strval($main_user));
        echo "<script>console.log('backup')</script>";
    }
    if($json == "" or $json == FALSE){
        echo "<script>alert('The server is being updated')</script>";
    }
    $allId = json_decode($json);
    foreach($allId as $aid){
        if(in_array($aid->id, $shadowIDs)){
            $allIdForUser [] = $aid;
        }
    }
}
...
<head>
    ...
    <script src="./js/script.js?v=114"></script>
</head>
    ...
    <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
            ...
            <div class="content-wrapper">
                ...
                if ($countOfId > 0) {
                    foreach ($ids as $id) {
                        $api_last_url = $api_last_url_address . $id["id"];
                        $json_last_url = @file_get_contents($api_last_url);
                        $info =json_decode(@file_get_contents("https://hivaind.ir/wil/informationID.php?id=". $id["id"]));
                        $datasForthisId = json_decode($json_last_url);
                        // echo"<script>console.log('datas for this id',".json_encode($datasForthisId).")</script>";
                        ?>
                        <div class="row ">
                                ...
                                <div class="card rounded-1">
                                    <div class="card-body ">
                                        ...
                                        <div class="row">
                                            <?php
                                            if($id["input"]!=null){
                                                $json_last_url = @file_get_contents("https://hivaind.ir/alarmPlus/Edit-configAPI.php?id=" . $id["id"]);
                                            ...
                                            foreach ($id["input"] as $in) {
                                                ...
                                                $en = $editconfig->$en_name;
                                                $endevice = $editconfig->enDevice;
                                                $thisInputValue = floatval(floatval($datasForthisId->$in) / 100);
                                                // echo "<script>console.log('thisInputValue',".json_encode($thisInputValue).")</script>";
                                                ?>
                                                <div class="col-xl-3 col-sm-6 grid-margin stretch-card">
                                                    <div class="card">
                                                        <script>
                                                            console.log('<?php echo "id: " . $id["id"] . "input: " . $in . "value: $thisInputValue min: $min max: $max en: $en";?>');
                                                        </script>
                                                        <div class="card-body bg-card-sp rounded" style="<?php if($endevice == '1' and $en == '1' and $thisInputValue > -99.00){if($thisInputValue<$min){echo "box-shadow:  0 0 10px rgba(220, 53, 69, 0.4),0 6px 35px rgba(220, 53, 69, 0.4);";}else if($thisInputValue>$max){echo "box-shadow:  0 0px 10px rgba(13, 110, 253, 0.4),0 0px 35px 0 rgba(13, 110, 253, 0.4);";}}?> ">
                                                            <div class="row">
                                                                <div class="col-9">
                                                                    <div class="d-flex align-items-center align-self-start">
                                                                        <?php 
                                                                                
                                                                                $cname = 'c'. substr($in, 5, 1);
                                                                                if(substr($in, 5,1) == 'A' or substr($in, 5,1) == 'B' or substr($in, 5,1) == 'C' or substr($in, 5,1) == 'D'){
                                                                                    $thisInputValue = intval($thisInputValue) + intval($info->$cname);
                                                                                }
                                                                                $thisInputValue = floatval(floatval($datasForthisId->$in) / 100);?>
                                                                        <div class="mb-0 fs-4" name="txtInputValue" id="<?php echo $id["id"].''.$in?>">
                                                                            <?php
                                                                            if($id['id'] == '115' or $id['id'] == 115){
                                                                                // echo "<script>alert('id=115')</script>";
                                                                                echo floatval((floatval($datasForthisId->$in)*10) / 100); 
                                                                            }else{
                                                                                if ($thisInputValue < -90) {
                                                                                    echo "--";
                                                                                }else{
                                                                                    // echo  "<script>console.log('".$cname." : ".$info->$cname."')</script>";
                                                                                    echo $thisInputValue;
                                                                                }
                                                                            }
